...
<div class="notebox", style="background-color: #E6E6E6;"> 
<b>Note:</b>  When using <code>p_load</code> and friends inside of functions the user may want to explicitly supply the package name(s) as character strings and set <code>character.only = FALSE</code>.
</div>
...
where the `...` argument is  allows the user to pass inquoted package names.  USers may also supply `"all"` to `...`  to remove all add on packages.  The `negate` argument negates, unloading all packages except those supplied to `...`.
