...
function (..., negate = FALSE) 
{
    ...
    toUnloadButCantUnload <- toUnloadButNotLoaded[toUnloadButNotLoaded %in% c("base", defaultPackages)]
    toUnloadButNotLoaded <- toUnloadButNotLoaded[!toUnloadButNotLoaded %in% c("base", defaultPackages)]
    mt <- toUnload %in% "all"
